...
export const loader = async ({ request, context }: LoaderFunctionArgs) => {
  ...
  console.log("Context structure in loader:", getDurableObjectDebugInfo(context));
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  if (action === "updateSettings") {
    ...
    // Map UI database names to our service keys (now matches directly)
    ...
    const dbKey = dbKeyMap[dbTarget] || "DB1";
    const result = await updateDatabaseSetting(context, dbKey, isChecked);
    
    // Return complete result with dbTarget for UI consistency
    return Response.json({
      ...result,
      dbTarget
    });
...
/**
 * Disable automatic revalidation since action provides all needed data
 */
 ...
 */
export default function DurableExample() {
  ...
  const handleCheckboxChange = (dbTarget: string, checked: boolean) => {
    ...
    if (dbAvailable) {
      ...
      // Submit the form using the fetcher - no revalidation needed
      fetcher.submit(formData, { method: "post" });
      
      // Show a success toast notification
      shopify.toast.show(`Setting saved to ${dbTarget}`);
    } else {
      // Revert optimistic update if DB not available
      if (dbTarget === "DB3") {
        setCheckboxStateDB3(!checked);
      } else if (dbTarget === "DB2") {
        setCheckboxStateDB2(!checked);
      ...
      // Show warning if durable object is not available
      shopify.toast.show(`Durable Object ${dbTarget} not available, setting not saved`);
    }
  };
    ...
    <BlockStack gap="200">
      ...
      {!isAvailable && (
        <Text as="p" variant="bodyMd" color="subdued">
          Note: Durable Object {dbName} is not available. {dbName === "DB1" ? db1.error : dbName === "DB2" ? db2.error : db3.error}
        </Text>
      )}
    </BlockStack>
  );
   ...
   */
  const renderDatabaseTable = (dbName: string, instanceName: string, tableData: any[], isAvailable: boolean, error: string | null) => (
    <BlockStack gap="400">
      <Text as="h3" variant="headingSm">
        {dbName}: {instanceName} (Durable Object Storage)
      </Text>
      {isAvailable ? (
        <>
  ...
  return (
    <Page title="Durable Object Storage Example">
      <Layout>
          ...
          <Card>
            <BlockStack gap="400">
              <Text as="h2" variant="headingMd">
                Durable Object Database Settings
              </Text>
              <Text as="p" variant="bodyMd">
                Toggle these checkboxes to write values to each durable object storage instance. Changes will be reflected in the table below.
              </Text>
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="600">
              <Text as="h2" variant="headingMd">
                Durable Object Storage Contents
              </Text>
              
              {/* DB1 Contents */}
              {renderDatabaseTable("DB1", db1.instanceName, tableDataDB1, db1.dbAvailable, db1.error)}
              
              {/* Divider */}
              <div style={{ borderBottom: '1px solid #ddd', width: '100%' }}></div>
              
              {/* DB2 Contents */}
              {renderDatabaseTable("DB2", db2.instanceName, tableDataDB2, db2.dbAvailable, db2.error)}
              
              {/* Divider */}
              <div style={{ borderBottom: '1px solid #ddd', width: '100%' }}></div>
              
              {/* DB3 Contents */}
              {renderDatabaseTable("DB3", db3.instanceName, tableDataDB3, db3.dbAvailable, db3.error)}
            </BlockStack>
          </Card>
        </Layout.Section>
      </Layout>
